...
def load_in_all_main_lut(max_isd_distance):
    '''
    Load in .csv file from the simulation results in order to visualize as graph
    Load the specific file by updating the 'filenames' with directory to that file
    '''
    ...
    output['capacity_mbps_m2'] = (output['capacity_mbps_km2'] / 1e6)
...
def plotting_function1_isd(data):
    '''
    Line graphs of all site radius/ISD
    '''
...
#        palette=sns.color_palette("husl", 6),
        palette=sns.color_palette('bright', 2), # Configure another color palette based on your demand
    ...
    plot.axes[5].set_ylabel('Capacity (Mbps/m^2)')
...
def plotting_function2_isd(data):
    '''
    Bar graphs of specific site radius/ISD (Ex. Site radius = 340m)
    '''
...
#        palette=sns.color_palette("husl", 6),
    ...
    plot.axes[3].set_ylabel('SINR (dB)')
    plot.axes[4].set_ylabel('SE (Bps/Hz)')
    plot.axes[5].set_ylabel('Capacity (Mbps)')
